...
async function main() {
  ...
  const factory = await ethers.getContractFactory("CrashVault");
  ...
  fs.writeFileSync("crash-vault-deployment.json", JSON.stringify(payload, null, 2));
  console.log("CrashVault deployed:", address);
}
